 ...
 * @version   2.6.1 for parent theme Boilerplate for publication on WordPress.org
	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			$this->strings = array(
				...
				'complete'                        => esc_html__( 'All plugins installed and activated successfully. %1$s', '__theme_txtd' ),
				'dismiss'                         => esc_html__( 'Dismiss this notice', '__theme_txtd' ),
				'notice_cannot_install_activate'  => esc_html__( 'There are one or more required or recommended plugins to install, update or activate.', '__theme_txtd' ),
				'contact_admin'                   => esc_html__( 'Please contact the administrator of this site for help.', '__theme_txtd' ),
			);
		...
		public function filter_plugin_action_links_update( $actions ) {
			$actions['update'] = sprintf(
				...
				esc_attr__( 'This plugin needs to be updated to be compatible with your theme.', '__theme_txtd' ),
				esc_html__( 'Update Required', '__theme_txtd' )
			);
		...
		protected function do_plugin_install() {
			...
			if ( ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) || ( isset( $_GET['tgmpa-update'] ) && 'update-plugin' === $_GET['tgmpa-update'] ) ) {
				...
				if ( $this->is_tgmpa_complete() ) {
					echo '<p>', sprintf( esc_html( $this->strings['complete'] ), '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', '__theme_txtd' ) . '</a>' ), '</p>';
			...
			if ( ! empty( $message ) || $total_required_action_count > 0 ) {
				...
				if ( ! current_user_can( 'activate_plugins' ) && ! current_user_can( 'install_plugins' ) && ! current_user_can( 'update_plugins' ) ) {
					...
					foreach ( $message as $type => $plugin_group ) {
						...
						$imploded       = empty( $linked_plugins ) ? $last_plugin : ( implode( ', ', $linked_plugins ) . ' ' . esc_html_x( 'and', 'plugin A *and* plugin B', '__theme_txtd' ) . ' ' . $last_plugin );

						$rendered .= sprintf(
							$line_template,
							sprintf(
								translate_nooped_plural( $this->strings[ $type ], $count, '__theme_txtd' ),
								$imploded,
								$count
							)
						);
		...
		protected function create_user_action_links_for_notice( $install_count, $update_count, $activate_count, $line_template ) {
			...
			if ( current_user_can( 'install_plugins' ) ) {
				if ( $install_count > 0 ) {
					$action_links['install'] = sprintf(
						$link_template,
						translate_nooped_plural( $this->strings['install_link'], $install_count, '__theme_txtd' ),
				...
				if ( $update_count > 0 ) {
					$action_links['update'] = sprintf(
						$link_template,
						translate_nooped_plural( $this->strings['update_link'], $update_count, '__theme_txtd' ),
						esc_url( $this->get_tgmpa_status_url( 'update' ) )
					);
				}
			}
			...
			if ( current_user_can( 'activate_plugins' ) && $activate_count > 0 ) {
				$action_links['activate'] = sprintf(
					$link_template,
					translate_nooped_plural( $this->strings['activate_link'], $activate_count, '__theme_txtd' ),
		...
		public function show_tgmpa_version() {
			...
			esc_html(
				...
				/* translators: %s: version number */
					esc_html__( 'TGMPA v%s', '__theme_txtd' ),
					self::TGMPA_VERSION
				)
			),
			'</small></strong></p>';
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function get_plugin_advise_type_text( $required ) {
			if ( true === $required ) {
				return esc_html__( 'Required', '__theme_txtd' );
			}

			return esc_html__( 'Recommended', '__theme_txtd' );
		}
		...
		protected function get_plugin_source_type_text( $type ) {
			...
			switch ( $type ) {
				case 'repo':
					$string = esc_html__( 'WordPress Repository', '__theme_txtd' );
					break;
				case 'external':
					$string = esc_html__( 'External Source', '__theme_txtd' );
					break;
				case 'bundled':
					$string = esc_html__( 'Pre-Packaged', '__theme_txtd' );
					break;
			}
		...
		protected function get_plugin_status_text( $slug ) {
			if ( ! $this->tgmpa->is_plugin_installed( $slug ) ) {
				return esc_html__( 'Not Installed', '__theme_txtd' );
			}

			if ( ! $this->tgmpa->is_plugin_active( $slug ) ) {
				$install_status = esc_html__( 'Installed But Not Activated', '__theme_txtd' );
			} else {
				$install_status = esc_html__( 'Active', '__theme_txtd' );
			}

			$update_status = '';

			if ( $this->tgmpa->does_plugin_require_update( $slug ) && false === $this->tgmpa->does_plugin_have_update( $slug ) ) {
				$update_status = esc_html__( 'Required Update not Available', '__theme_txtd' );

			} elseif ( $this->tgmpa->does_plugin_require_update( $slug ) ) {
				$update_status = esc_html__( 'Requires Update', '__theme_txtd' );

			} elseif ( false !== $this->tgmpa->does_plugin_have_update( $slug ) ) {
				$update_status = esc_html__( 'Update recommended', '__theme_txtd' );
			}
			...
			/* translators: 1: install status, 2: update status */
				esc_html_x( '%1$s, %2$s', 'Install/Update Status', '__theme_txtd' ),
				$install_status,
				$update_status
			);
		}
			...
			foreach ( $this->view_totals as $type => $count ) {
				...
				switch ( $type ) {
					case 'all':
						...
						$text = _nx( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', $count, 'plugins', '__theme_txtd' );
						break;
					case 'install':
						/* translators: 1: number of plugins. */
						$text = _n( 'To Install <span class="count">(%s)</span>', 'To Install <span class="count">(%s)</span>', $count, '__theme_txtd' );
						break;
					case 'update':
						/* translators: 1: number of plugins. */
						$text = _n( 'Update Available <span class="count">(%s)</span>', 'Update Available <span class="count">(%s)</span>', $count, '__theme_txtd' );
						break;
					case 'activate':
						/* translators: 1: number of plugins. */
						$text = _n( 'To Activate <span class="count">(%s)</span>', 'To Activate <span class="count">(%s)</span>', $count, '__theme_txtd' );
						break;
					default:
						$text = '';
						break;
		...
		public function column_version( $item ) {
			...
			if ( $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
				$installed = ! empty( $item['installed_version'] ) ? $item['installed_version'] : esc_html_x( 'unknown', 'as in: "version nr unknown"', '__theme_txtd' );
				...
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;%1$s">%2$s</span>' . esc_html__( 'Installed version:', '__theme_txtd' ) . '</p>',
					$color,
					$installed
				);
			}
			...
			if ( ! empty( $item['minimum_version'] ) ) {
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;">%1$s</span>' . esc_html__( 'Minimum required version:', '__theme_txtd' ) . '</p>',
			...
			if ( ! empty( $item['available_version'] ) ) {
				...
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;%1$s">%2$s</span>' . esc_html__( 'Available version:', '__theme_txtd' ) . '</p>',
					$color,
		...
		public function no_items() {
			echo esc_html__( 'No plugins to install, update or activate.', '__theme_txtd' ) . ' <a href="' . esc_url( self_admin_url() ) . '"> ' . esc_html__( 'Return to the Dashboard', '__theme_txtd' ) . '</a>';
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		}
		...
		public function get_columns() {
			$columns = array(
				...
				'plugin' => esc_html__( 'Plugin', '__theme_txtd' ),
				'source' => esc_html__( 'Source', '__theme_txtd' ),
				'type'   => esc_html__( 'Type', '__theme_txtd' ),
			);

			if ( 'all' === $this->view_context || 'update' === $this->view_context ) {
				$columns['version'] = esc_html__( 'Version', '__theme_txtd' );
				$columns['status']  = esc_html__( 'Status', '__theme_txtd' );
			}
		...
		protected function get_row_actions( $item ) {
			...
			if ( ! $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
				...
				$actions['install'] = esc_html__( 'Install %2$s', '__theme_txtd' );
			} else {
				// Display the 'Update' action link if an update is available and WP complies with plugin minimum.
				if ( false !== $this->tgmpa->does_plugin_have_update( $item['slug'] ) && $this->tgmpa->can_plugin_update( $item['slug'] ) ) {
					/* translators: %2$s: plugin name in screen reader markup */
					$actions['update'] = esc_html__( 'Update %2$s', '__theme_txtd' );
				}

				// Display the 'Activate' action link, but only if the plugin meets the minimum version.
				if ( $this->tgmpa->can_plugin_activate( $item['slug'] ) ) {
					/* translators: %2$s: plugin name in screen reader markup */
					$actions['activate'] = esc_html__( 'Activate %2$s', '__theme_txtd' );
				}
			}
		...
		public function wp_plugin_update_row( $slug, $item ) {
			...
			esc_html__( 'Upgrade message from the plugin author:', '__theme_txtd' ),
		...
		public function get_bulk_actions() {
			...
			if ( 'update' !== $this->view_context && 'activate' !== $this->view_context ) {
				if ( current_user_can( 'install_plugins' ) ) {
					$actions['tgmpa-bulk-install'] = esc_html__( 'Install', '__theme_txtd' );
				}
			}

			if ( 'install' !== $this->view_context ) {
				if ( current_user_can( 'update_plugins' ) ) {
					$actions['tgmpa-bulk-update'] = esc_html__( 'Update', '__theme_txtd' );
				}
				if ( current_user_can( 'activate_plugins' ) ) {
					$actions['tgmpa-bulk-activate'] = esc_html__( 'Activate', '__theme_txtd' );
				}
			}
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action() ) {
				...
				if ( empty( $_POST['plugin'] ) ) {
					if ( 'install' === $install_type ) {
						$message = esc_html__( 'No plugins were selected to be installed. No action taken.', '__theme_txtd' );
					} else {
						$message = esc_html__( 'No plugins were selected to be updated. No action taken.', '__theme_txtd' );
					}
				...
				if ( empty( $plugins_to_install ) ) {
					if ( 'install' === $install_type ) {
						$message = esc_html__( 'No plugins are available to be installed at this time.', '__theme_txtd' );
					} else {
						$message = esc_html__( 'No plugins are available to be updated at this time.', '__theme_txtd' );
					}
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( empty( $_POST['plugin'] ) ) {
					echo '<div id="message" class="error"><p>', esc_html__( 'No plugins were selected to be activated. No action taken.', '__theme_txtd' ), '</p></div>';
				...
				if ( empty( $plugins_to_activate ) ) {
					echo '<div id="message" class="error"><p>', esc_html__( 'No plugins are available to be activated at this time.', '__theme_txtd' ), '</p></div>';
			...
			if ( ! class_exists( 'TGMPA_Bulk_Installer' ) ) {
				...
				class TGMPA_Bulk_Installer extends Plugin_Upgrader {
					...
					public function activate_strings() {
						$this->strings['activation_failed']  = esc_html__( 'Plugin activation failed.', '__theme_txtd' );
						$this->strings['activation_success'] = esc_html__( 'Plugin activated successfully.', '__theme_txtd' );
					}
					...
					public function bulk_footer() {
						...
						if ( $this->tgmpa->is_tgmpa_complete() ) {
							...
							$update_actions['dashboard'] = sprintf(
								...
								'<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', '__theme_txtd' ) . '</a>'
							);
						} else {
