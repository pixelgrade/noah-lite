...
if ( $wpGrade_importerError !== false ) {
	$response['id'] = new WP_Error( 'import_theme_options_noscript', 'The Auto importing script could not be loaded. Please use the <a href="' . esc_url( admin_url( 'import.php' ) ) . '">WordPress default import</a> and import the .XML file provided in the archive you\'ve received on purchase manually.' );
} else {
	if ( class_exists( 'WPGrade_WP_Import' ) ) {
		include_once( 'wordpress-importer/wpgrade-import-class.php' );
	}
	if ( ! is_file( $import_filepath . '.php' ) ) {
		$response['id'] = new WP_Error( 'import_theme_options_nofile', 'The PHP file containing the theme options data could not be found or could not be read in <pre>' . get_template_directory() . 'inc/import/demo-data' . '</pre><br/> You might want to try to set the file permission to 777.<br/>If this doesn\'t work please use the Theme Options Import/Export and import the .txt file provided in the archive you\'ve received on purchase manually.' );
	} else {
		ob_start();
